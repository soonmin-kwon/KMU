  ...
  for (int i = 0; i < scene->mNumMaterials; i++) 
    ...
    for (unsigned int j = 0; j < diffuseTextureCount; j++) 
    {
          ...
          j, &textureFilePath)) 
      {
        std::string bp;
        bp = basepath;
        bp = bp.append(textureFilePath.data);
        const char *fp = bp.c_str();
        image = stbi_load(fp, &width,&height,&channels,STBI_rgb);
...
void draw_mesh(const aiMesh* mesh, const aiMatrix4x4& mat_model)
{
  ...
  glUniform4f(loc_u_light_diffuse, light_diffuse.r,light_diffuse.g,light_diffuse.b,light_diffuse.a);
  ...
  glUniform4f(loc_u_material_specular, material_specular.a,material_specular.g,material_specular.b,material_specular.a);
  ...
  for(int i=0; i< meshes.size(); i++){
    ...
    glActiveTexture(GL_TEXTURE0 + mesh.material_index);
    glBindTexture(GL_TEXTURE_2D, mesh.material_index+1);
    glUniform1i(loc_u_diffuse_texture, mesh.material_index);
    ...
    for( int j=0; j< mesh.faces.size(); j++){
      ...
      if(mesh.has_texture){
        ...
        glVertexAttribPointer(loc_a_texcoord,3,GL_FLOAT, GL_FALSE, 3*sizeof(float),(void*)0 );
      }
